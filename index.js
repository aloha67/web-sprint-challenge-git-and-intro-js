...
function listOfNames(artistArray) {
  const copyArtists = artistArray.slice();
...
const artistNames = listOfNames(artists)
...
function removeArtist(artistArray, index) {
  const copiedArray = artistArray.slice();
  copiedArray.splice(index, 1)
  return copiedArray
}

//const newArtists = removeArtist(artists, 2);
...
function addArtist(artistsArray, name, years, genre, nationality, bio) {
  const newArray = artistsArray.slice();
...
const newArtists = addArtist(artists, "John Andrade", "1994", "Web Design", "Mexican", "lorem ipsum");
...
//console.log(artists, 'Vince Van Gogh')
...
function artistByCountry(artists, nationality) {
  return artists.filter(artist => artist.nationality === nationality && !Array.isArray(artist.nationality)).map(artist => artist.name);
}
